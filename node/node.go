...
type Node struct {
	...
	lock          sync.Mutex
	lifecycles    []node.Lifecycle // All registered backends, services, and auxiliary services that have a lifecycle
...
func (n *Node) Start() error {
	...
	err := n.openEndpoints()
	lifecycles := make([]node.Lifecycle, len(n.lifecycles))
...
func containsLifecycle(lfs []node.Lifecycle, l node.Lifecycle) bool {
	for _, obj := range lfs {
		if obj == l {
			return true
		}
...
// stopServices terminates running services, RPC and indexer.
// It is the inverse of Start.
func (n *Node) stopServices(running []node.Lifecycle) error {
	n.stopRPC()

	// Stop running lifecycles in reverse order.
	failure := &node.StopError{Services: make(map[reflect.Type]error)}
